...
/*
gcc -g -I${CWPROOT}/include -O -fPIC -std=c99 -Wall -pedantic -Wno-long-long  -D_FILE_OFFSET_BITS=64 -D_LARGEFILE64_SOURCE -DCWP_LITTLE_ENDIAN -DSUXDR  -D_BSD_SOURCE -D_POSIX_SOURCE su_fxdecon.c -L${CWPROOT}/lib -lsu -lpar -lcwp -lm  -o $HOME/bin/su_fxdecon

Didn't work:
gcc su_fxdecon.c -I$CWPROOT/include -O3 -Wall -Wno-long-long -ansi -pedantic -D_POSIX_SOURCE -m32 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE -DGNU_SOURCE -DCWP_LITTLE_ENDIAN -L$CWPROOT/lib -lsu -lpar -lcwp -lm  -o $HOME/bin/su_fxdecon
*/
  ...
  for (iw=0;iw<ntw;iw++) {
    ...
    for (jw=0;jw<ntrc;jw++) {
      ...
      for (ifq = 0; ifq < nf; ifq++) { 
        ...
        /*        amp = sqrt( fdata[jw][ifq].r*fdata[jw][ifq].r + fdata[jw][ifq].i*fdata[jw][ifq].i );
        phase = atan2( -fdata[jw][ifq].i, fdata[jw][ifq].r );
        fprintf(stdout,"%d %f  %f  %f  %f  %f\n",jw,ifq*d1,fdata[jw][ifq].r,fdata[jw][ifq].i,amp,phase); */
      }
    }
    ...
    for (jx=0;jx<nwx;jx++){
      ...
      for (jw=0;jw<ntrwu+2*ntrf;jw++) {
        ...
        fprintf(stdout, "%d %d  %f %f\n", jw, ifq, fdataw[jw][ifq].r, fdataw[jw][ifq].i );
      }
      /* loop over frequencies */
      for (ifq=0;ifq<nf;ifq++) {
