...
namespace mod_fft_2d {
  struct VariableStruct {
    ...
    double* realBuffer;           // Double buffer 
    fftw_complex* complexBuffer; // Complex buffer
    fftw_plan  myPlan;           // FFTW plan (presumably a structure?)
  ...
  csLogWriter* writer )
{
  ...
  if ( direction == FORWARD ){
    ...
    if ( recalc || vars->firstCall ){
      ...
      // Buffer allocations
      fftR = (double*)fftw_malloc(sizeof(double) * ntp*nxp);
      fftC = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) *nxp*nfreq);
      fftPlan2D = fftw_plan_dft_r2c_2d(nxp, ntp, fftR, fftC, vars->fftw_flag);
    ...
    memset (fftR, 0, ntp*nxp*sizeof(double));
    ...
    for(itrc=0; itrc<numTracesIn; itrc++ ) {
      ...
      if (itrc%2 !=0)
        for(isamp=0; isamp<nt; isamp++)  {
          samples[isamp] *= -1;
          fftR[itrc*ntp+isamp] = samples[isamp];
        }
      //      memcpy (&fftR[itrc*ntp], samples, nt*sizeof(float));
    }
  
    // FFT
    memset (fftC, 0, sizeof(fftw_complex)*nxp*nfreq);
    fftw_execute(fftPlan2D);
    ...
    if ( output == FK_COMPLEX ){
      int nOut = nfreq*2;
      double* out = (double*)(fftC);
      for(itrc=0; itrc<nxp; itrc++ ){
        float *samples = traceGather->trace(itrc)->getTraceSamples();
        for( int isamp = 0; isamp < nOut; isamp++ ) {
          samples[isamp] = out[itrc*nOut+isamp];
        }
      ...
      for(itrc=0; itrc<nxp; itrc++ ){
        float *samples = traceGather->trace(itrc)->getTraceSamples();
        fftw_complex *bufC = fftC + itrc*nfreq;
        for (isamp=0; isamp<nfreq; isamp++) {
  ...
  if ( direction == INVERSE ){
    ...
    if ( recalc ){
      ...
      fftR = (double*)fftw_malloc(sizeof(double) * ntp*nxp);
      fftC = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) *nxp*nfreq);    
      fftPlan2D = fftw_plan_dft_c2r_2d(nxp, ntp, fftC, fftR, vars->fftw_flag);
    ...
    if ( output == FK_COMPLEX ){
      ...
      for(itrc=0; itrc<nx; itrc++ ){
        float *samples = traceGather->trace(itrc)->getTraceSamples();
        for( int isamp = 0; isamp < nOut; isamp++ ) {
          out[itrc*nOut+isamp] = samples[isamp];
        }
